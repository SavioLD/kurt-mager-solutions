...
(function () {
  ...
  function submit() {
    ...
    const _drawBox = files.length ? `<div style="margin:16px 0 2px;padding:15px 17px;background:var(--green-50);border:1px solid #cdeccf;border-radius:14px;text-align:left">
          <b style="display:block;margin-bottom:6px">📎 Ihre Zeichnung${files.length > 1 ? "en" : ""}: ${esc(_fn)}</b>
          <p style="font-size:.9rem;color:var(--slate);margin:0 0 12px;line-height:1.5">Bitte senden Sie Ihre Zeichnung${files.length > 1 ? "en" : ""} noch kurz per E-Mail an uns – ein Klick, Datei anhängen, absenden. So kommt sie sicher bei uns an.</p>
    ...
    const payload = {
      ...
      Zeichnung: state.file ? state.file + " — folgt per E-Mail" : "—",
      Liefertermin: state.deadline || "—",
