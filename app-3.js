...
(function () {
  ...
  function renderFiles() {
    ...
    if (status) {
      ...
      status.textContent = n === 0
        ...
        : n === 1
          ? "1 Zeichnung ausgewählt: " + files[0].name
          : n + " Zeichnungen ausgewählt";
  ...
  function submit() {
    ...
    const _fn = files.map((f) => f.name).join(", ");
    const _canAttach = files.length > 0 && !files.some((f) => f.size > 5 * 1024 * 1024) && files.reduce((s, f) => s + f.size, 0) <= 15 * 1024 * 1024;
    const _drawMail = "mailto:info@mager-solutions.de?subject=" + encodeURIComponent("Zeichnung zu Anfrage " + ref) + "&body=" + encodeURIComponent("Guten Tag,\n\nanbei meine Zeichnung(en) zur Anfrage " + ref + ".\nDateien: " + _fn + "\n\n(Bitte die Datei(en) an diese E-Mail anhängen.)\n\nViele Grüße\n" + state.name);
    const _drawBox = files.length ? `<div id="kmDrawBox" style="display:none;margin:16px 0 2px;padding:15px 17px;background:var(--green-50);border:1px solid #cdeccf;border-radius:14px;text-align:left">
          <b style="display:block;margin-bottom:6px">📎 Ihre Zeichnung${files.length > 1 ? "en" : ""}: ${esc(_fn)}</b>
          <p style="font-size:.9rem;color:var(--slate);margin:0 0 12px;line-height:1.5">Bitte senden Sie Ihre Zeichnung${files.length > 1 ? "en" : ""} noch kurz per E-Mail an uns – ein Klick, Datei anhängen, absenden.</p>
          <a class="btn btn--primary" href="${_drawMail}">Zeichnung${files.length > 1 ? "en" : ""} per E-Mail senden</a>
        </div>` : "";
    body.innerHTML = `<div class="done">
        ...
        <div class="done__actions">
          <button type="button" class="btn ${files.length ? "btn--ghost" : "btn--primary"}" id="restart">Neue Anfrage starten</button>
        </div>
      </div>`;
    $("#restart").addEventListener("click", reset);
    if (window.confettiBurst) window.confettiBurst();
    const revealDraw = () => { const bx = document.getElementById("kmDrawBox"); if (bx) bx.style.display = ""; };
    const fields = {
      ...
      Zeichnung: state.file ? state.file + (_canAttach ? " (siehe Anhang)" : " — folgt per E-Mail") : "—",
      Liefertermin: state.deadline || "—",
    ...
    // multipart/form-data: ermöglicht echte Datei-Anhänge (Web3Forms PRO). Keine Header setzen – der Browser ergänzt den multipart-Boundary selbst.
    const fd = new FormData();
    Object.keys(fields).forEach((k) => fd.append(k, fields[k]));
    if (_canAttach) files.forEach((f, i) => fd.append(i === 0 ? "attachment" : "attachment_" + (i + 1), f, f.name));
    fetch("https://api.web3forms.com/submit", { method: "POST", body: fd })
      .then((r) => r.json())
      .then((d) => {
        const m = document.getElementById("kmSendMsg");
        if (d && d.success) {
          if (m) m.textContent = "Ihre Anfrage ist bei uns eingegangen – Ihr fester Ansprechpartner meldet sich zeitnah mit einem passgenauen Angebot." + (_canAttach ? " Ihre Zeichnung" + (files.length > 1 ? "en wurden" : " wurde") + " mitgesendet." : "");
          if (files.length && !_canAttach) revealDraw();
        } else {
          ...
          console.error("Web3Forms Antwort:", d);
          if (files.length) revealDraw();
        }
      })
      .catch((err) => {
        const m = document.getElementById("kmSendMsg");
